...
class Layer:
    ...
    def activation(self, activation_input: NDArray) -> NDArray[float]:
        ...
        # softmax
          ...
          return (activation_input>=0) * activation_input
    ...
    def _get_weight_deltas(self, targets: NDArray, inputs: NDArray):
        ...
        for i in range(self.nodes_out):
            for j in range(self.nodes_in):
                weight_deltas[i][j] = deltas[i] * inputs[j] + 0.01 * self.weights[i][j]
    ...
    def learn_as_nn_layer(self, deltas: NDArray[float]):
        ...
        weights_delta = np.dot(deltas[:, np.newaxis] * self.activation_derivative(output=self.outputs)[:, np.newaxis],
...
class NeuralNetwork:
    ...
    def learn(self, inputs: NDint_float, targets: NDint_float) -> Tuple[float, NDArray] :
        ...
        end_time = time.time()
        #print(f"Time in forwad prop is {end_time - start_time}")
        # compute deltas
        ...
        end_time = time.time()
        #print(f"Time in back prop is {end_time - start_time}")

        return np.mean(self.get_error(prediction=prediction, targets=targets) ** 2), prediction
...
def classify(image : NDArray):
    ...
    neuralnet.layers[1].bias = file["bias_2"]
